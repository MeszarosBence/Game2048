...
//		Thread.sleep(500); //In order to verify it visually
	...
	public void testReadKey() throws Exception {
		...
		if (read == UP) System.out.println("UP was pressed");
		if (read == DOWN) System.out.println("DOWN was pressed");
		if (read == LEFT) System.out.println("LEFT was pressed");
		if (read == RIGHT) System.out.println("RIGHT was pressed");
	...
	public void testMoveOneValueLeft() throws Exception {
		int randomValue = 2;
		g.table[3][3] = randomValue;
		g.printTable();
		
		g.left();
		
		assertThat(g.table[3][0], is(randomValue));
		g.printTable();
	}
	...
	public void testMoveValuesLeftAndAddThemIfTheyAreEqual() throws Exception {
		int randomValue = 2;
		g.table[3][3] = randomValue;
		g.table[3][2] = randomValue;
		g.printTable();
		
		g.left();
		
		assertThat(g.table[3][0], is(2 * randomValue));
		g.printTable();
	}
